...
function App() {
  ...
  const loadBlogPosts = async () => {
    try {
      ...
      if (sheetsPosts && sheetsPosts.length > 0) {
        console.log(`✅ Loaded ${sheetsPosts.length} posts from Google Sheets`);
        setPosts(sheetsPosts);
        setGoogleSheetsConnected(true);
      } else {
        console.log('⚠️ No posts found in Google Sheets, using local fallback');
        setGoogleSheetsConnected(false);
      }
    } catch (error) {
      console.log('📦 Using fallback posts due to server connection issues');
      setGoogleSheetsConnected(false);
    }
  };
  ...
  const handleBlogEdit = () => {
    ...
    const inputPassword = prompt(language === 'en' ? 'Enter admin password:' : 'Ingresa la contraseña de administrador:');
    
    if (inputPassword === savedPassword) {
      setShowBlogEditor(true);
    } else if (inputPassword !== null) {
      alert(language === 'en' ? 'Incorrect password' : 'Contraseña incorrecta');
    }
  };
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
      ...
      {showBlogEditor && (
        <BlogEditor
          ...
          onToggleGoogleSheets={toggleGoogleSheets}
        />
      )}
